...
public class HeyWikiClient {
    ...
    public HeyWikiClient() {
        ...
        var platform = HeyWikiPlatform.get();

        this.config = HeyWikiConfig.load();

        platform.registerKeyMapping(openWikiKey);
        platform.registerKeyMapping(openWikiSearchKey);

        platform.registerClientCommand(HeyWikiClient::registerCommands);

        platform.onChatReceived(ChatWikiLinks::onClientChatReceived);

        DebugScreenEntriesMixin.invokeRegister(HeyWikiClient.id("url"), new HeyWikiDebugEntry());

        platform.onItemTooltip((stack, lines) -> {
        ...
        platform.onClientTickPost(Raycast::onClientTickPost);
        platform.onClientTickPost(WikiSearchScreen::onClientTickPost);

        this.familyManager = new WikiFamilyManager();
        this.translationManager = new WikiTranslationManager();
        platform.registerAssetReloadListener(id("family"), this.familyManager);
        platform.registerAssetReloadListener(id("translation"), this.translationManager);
    }

    private static void registerCommands(CommandDispatcher<SharedSuggestionProvider> dispatcher,

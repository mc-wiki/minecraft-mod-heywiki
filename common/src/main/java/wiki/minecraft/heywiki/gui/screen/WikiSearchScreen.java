...
public class WikiSearchScreen extends Screen {
    ...
    public WikiSearchScreen() {
        super(Component.translatable("gui.heywiki_search.title",
    ...
    @Override protected void init() {
        ...
        mainLayout.addChild(new StringWidget(Component.translatable("gui.heywiki_search.title",
        ...
        this.textField = this.textField == null
                ? new EditBox(this.font, 175, 20, Component.translatable("gui.heywiki_search.title",
                                                                         Component.translatable(
                                                                                          wikiFamily.getTranslationKey())
                                                                                  .withStyle(
                                                                                          Style.EMPTY.withUnderlined(
                                                                                                  true))))
                : this.textField;
        ...
        textFieldLayout.addChild(Button.builder(Component.translatable("gui.heywiki_search.switch_wiki"), button -> {
            assert this.minecraft != null;
            this.minecraft.setScreen(new WikiSelectScreen(this, MOD.familyManager().getAvailableFamilies().stream()
                                                                   .filter(family -> family.getWiki()
                                                                                           .searchProviderType()
                                                                                           .isPresent()).sorted()
                                                                   .toList(),
        ...
        wiki.searchProvider().ifPresentOrElse(url -> omniboxLayout.addChild(this.entryList), () -> {
            var layout = omniboxLayout.addChild(LinearLayout.vertical().spacing(4));
            layout.defaultCellSetting().alignHorizontallyCenter();
            layout.addChild(new StringWidget(Component.translatable("gui.heywiki_search.no_suggestions"), this.font));
            layout.addChild(Button.builder(Component.translatable("gui.heywiki_search.search"), button -> {
